...
test_that("dq_render_handsontable works properly with different parameters", {
  ...
  expect_silent(dq_render_handsontable("id", df, page_size = 78))
  ...
  expect_silent(dq_render_handsontable("id", df, table_params = NULL))
  expect_silent(dq_render_handsontable("id", df, col_params = NULL))
  expect_silent(dq_render_handsontable("id", df, col1 = list(type = "text")))
  rV <- shiny::reactiveValues(id = df)
  expect_silent(dq_render_handsontable("id", rV, page_size = 78))
})
