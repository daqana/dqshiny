...
test_that("null works", {
  res <- shiny::fluidRow(class = "filter-row")
  expect_equal(dqshiny:::filter_row(NULL, NULL), res)
  expect_equal(dqshiny:::filter_row(NULL, NULL, reset = F), res)
})

test_that("all filters work for empty data", {
  df <- data.frame(A = character(0), B = logical(0), C = as.Date(character(0)), D = numeric(0), stringsAsFactors = F)
  expect_silent(dqshiny:::filter_row(NULL, df))
  expect_silent(dqshiny:::filter_row(NULL, df, reset = F))
...
test_that("all filters work for proper data", {
  df <- data.frame(A = rep("hello", 20), B = rep(c("huhu", "haha"), 10), C = 1:20, D = Sys.Date() - 0:19, stringsAsFactors = F)
  expect_silent(dqshiny:::filter_row(NULL, df))
  expect_silent(dqshiny:::filter_row(NULL, df, reset = F))
  expect_silent(dqshiny:::filter_row(NULL, df, "s"))
  expect_silent(dqshiny:::filter_row(NULL, df, "R"))
