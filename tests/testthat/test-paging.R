...
session <- readRDS(file.path("data", "shinySession.RData"))
test_that("update_page works with NULL inputs", {
  expect_true(grepl("paging-row", dqshiny:::paging_row(NULL, NULL, NULL)))
  expect_true(grepl("num_test_page", dqshiny:::paging_row("test", NULL, NULL)))
})

test_that("paging_row works with proper inputs", {
  expect_true(grepl("value=\"9\"", dqshiny:::paging_row("test", 2, 9)))
})
